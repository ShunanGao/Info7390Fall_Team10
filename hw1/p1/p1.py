...
@app.route('/<params>')
def default(params):
    return redirect(url_for('.do_foo', params=params))
...
def do_foo():
    ...
    try:
        ...
        formname = form[6:10]  # Form 10-Q - Quarterly report => 10-Q get the Type
        print(formname)
    ...
    for tab in refined_tables:
        ...
        with open(os.path.join(cvsDir, str(refined_tables.index(tab)) + 'tables.csv'), 'w') as f:
            writer = csv.writer(f)
    ...
    def zipdir(path, ziph, refined_tables):
        ...
        for tab in refined_tables:
            ziph.write(os.path.join(cvsDir, str(refined_tables.index(tab)) + 'tables.csv'))
    ...
    try:
        ...
        bucket_name = 'PartOne'.lower() + str(st).replace(" ", "").replace("-", "").replace(":", "").replace(".", "")
        bucket = conn.create_bucket(bucket_name, location=loc)
        print("bucket created")
        uploadFile = 'Problem1.zip'
